 ...
 */
class CouchbaseDriver implements \common_persistence_KvDriver{
    ...
    function connect($id, array $params)
    {
        if( !array_key_exists('cluster', $params) ){
            throw new PersistenceError('Couchbase driver not configured: cluster not set');
        }

        if( !array_key_exists('bucket', $params) ){
            throw new PersistenceError('Couchbase driver not configured: bucket not set');
        }
    ...
    {
        throw new PersistenceError( $exception->getMessage(), $exception->getCode() );
    }
